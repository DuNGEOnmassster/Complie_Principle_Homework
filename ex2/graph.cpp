...
//取负操作只有一个点
Agnode_t *genOneGraphNode(Agraph_t *g, struct ast_node *astnode, const std::string op_str)
{
    // 遍历子节点
    Agnode_t *child = graph_visit_ast_node(g, astnode);
    // 创建图形节点，设置Label为运算符
    std::string label_name = getNodeLabelName();
    Agnode_t *node = agnode(g, (char *)label_name.c_str(), 1);
    if (node != nullptr) {
...
// 根节点产生图片
Agnode_t *genBlockGraphNode(Agraph_t *g, struct ast_node *astnode)
{
...
Agnode_t *graph_visit_ast_node(Agraph_t *g, struct ast_node *astnode)
{
    ...
    case AST_OP_NEG:
        ...
        // 负号节点
        graph_node = genOneGraphNode(g, astnode->sons[0], "-");
