...
static bool ir_add(struct ast_node *node)
{
    ...
    if (left->val->type == ValueType::ValueType_Int && right->val->type == ValueType::ValueType_Int) {
...
static bool ir_neg(struct ast_node *node)
{
    ...
    if (left->val->type == ValueType::ValueType_Int) {
        Value *resultValue = newTempValue(ValueType::ValueType_Int);
        node->blockInsts.addInst(left->blockInsts);
        node->blockInsts.addInst(
              new BinaryIRInst(IRINST_OP_NEG, resultValue, left->val, nullptr));
        node->val = resultValue;
    } else {
        Value *resultValue = newTempValue(ValueType::ValueType_Real);
        node->blockInsts.addInst(left->blockInsts);
